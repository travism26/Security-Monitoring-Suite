...
show_help() {
    ...
    echo "  $0 -p log-aggregator    Archive only log-aggregator completed tasks"
    echo "  $0 -d 2024-Q4           Archive to the 2024-Q4 period"
    echo "  $0 -p siem-dashboard -d 2025-Q2  Archive siem-dashboard tasks to 2025-Q2"
}
...
archive_project_tasks() {
    ...
    if [ ! -f "$archive_file" ]; then
        echo "# $project Archived Tasks - $period" > "$archive_file"
        echo "" >> "$archive_file"
...
update_index_file() {
    ...
    if ! grep -q "$project.*$period" "$index_file"; then
        ...
        sed -i "${line_num}i| $project | $completed_task | $(date +%Y-%m-%d) | [$period](./archived/$project/$period.md) |" "$index_file"
    fi
}
